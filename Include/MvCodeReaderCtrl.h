...
/********************************************************************//**
 *  @fn     MV_CODEREADER_GIGE_GetGvcpTimeout
 *  @brief  获取GVCP命令超时时间（虚拟相机一直返回OK，但实际不支持，无效果）
 *  @param  handle                      [IN]            设备句柄
 ...
 *  @return Success, return MV_CODEREADER_OK. Failure, return error code
 ************************************************************************/
MV_CODEREADERCTRL_API int __stdcall MV_CODEREADER_GIGE_GetGvcpTimeout(IN void* handle, IN OUT unsigned int* pMillisec);

/********************************************************************//**
 *  @fn     MV_CODEREADER_SetFrameNodeNum
 *  @brief  设置帧数据缓存节点数量（虚拟相机一直返回OK，但实际不支持，无效果）
 *  @param  handle                      [IN]            设备句柄
 *  @param  nNodeNum                    [IN]            缓存节点数量，范围1-12，默认3
 *  @return 成功，返回MV_OK；错误，返回错误码 
 *  @remarks 创建句柄后，连接设备前调用有效
 
 *  @fn     MV_CODEREADER_SetFrameNodeNum
 *  @brief  Setting the number of frame data cache nodes
 *  @param  handle                      [IN]            Device handle
 *  @param  nMillisec                   [IN]            Cache node number, range:1-12, default:3
 *  @return Success, return MV_OK. Failure, return error code
 *  @remarks After creating the handle, before connecting to the device, it is valid to call this interface.
 ************************************************************************/
MV_CODEREADERCTRL_API int __stdcall MV_CODEREADER_SetFrameNodeNum(IN void* handle, IN unsigned int nNodeNum = 3);

/************************************************************************
*  @fn     MV_CODEREADER_InvalidateNodes()
*  @brief  清除GenICam节点缓存
*  @param  void* handle                    [IN]        相机句柄
*  @return 成功,返回MV_CODEREADER_OK,失败,返回错误码
...
*  @return Success, return MV_CODEREADER_OK. Failure, return error code																											
************************************************************************/
MV_CODEREADERCTRL_API int __stdcall MV_CODEREADER_InvalidateNodes(IN void* handle);
