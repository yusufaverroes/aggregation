...
// bool resizeRawImageBuffer(const unsigned char* srcImageData, int srcWidth, int srcHeight,
//                           unsigned char* &dstImageData, int dstWidth, int dstHeight) {
//     // Create cv::Mat objects for source and destination images
//     cv::Mat srcImage(srcHeight, srcWidth, CV_8UC1, (void*)srcImageData);
//     cv::Mat dstImage;
...
//     if (dstImage.empty()) {
//         std::cerr << "Error: Failed to resize image buffer" << std::endl;
//         return false;
//     }

//     // Allocate memory for the destination image buffer
//     size_t dstImageSize = dstWidth * dstHeight;
//     dstImageData = new unsigned char[dstImageSize];

//     // Copy resized image data from the cv::Mat to the destination image buffer
//     std::memcpy(dstImageData, dstImage.data, dstImageSize);
